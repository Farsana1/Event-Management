import wedding0 from "../assets/images/wedding-0.jpg";
...
const Wedding = () => {
  ...
  const images = [wedding0, wedding1, wedding2];

 /*  const getImagePath = (index) => `/assets/images/wedding-${index}.jpg`; */
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const weddingEvents = response.data.filter(event => event.type === 'wedding');
        setEvents(weddingEvents); // Assuming the backend sends an array of events
        setIsLoading(false);
      } catch (error) {
